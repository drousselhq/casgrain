...
```bash
gradle -p tests/fixtures/android-smoke/app assembleDebug
```

Then compile and run the fixture scenario:

```bash
cargo run --bin casgrain -- compile tests/fixtures/android-smoke/features/tap_counter.feature
cargo run --bin casgrain -- run-android-smoke tests/fixtures/android-smoke/features/tap_counter.feature
```
...
- `tests/fixtures/ios-smoke/` — smallest honest iOS simulator-backed smoke slice
