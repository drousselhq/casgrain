...
- `.github/runner-host-advisory-sources.json` is now the repo-owned contract for runner-host source-backed promotion decisions: `runner-images` is promoted on current `main`, while `android-java`, `android-gradle`, `android-emulator-runtime`, and `ios-xcode-simulator` remain `manual-review-required` follow-ups
- those expansions intentionally stay scoped to dependency-graph-backed alerts, the explicit checked-in security-tooling inventory, and the bounded runner-host release-metadata/drift lane rather than broad runner-image or generic downloaded-tool CVE scraping; later source-specific promotion work now remains in #154, #155, #156, and #144 while the shipped `runner-images` slice continues to reuse the existing runner-host managed-issue lane
