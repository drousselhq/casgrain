...
- `.github/runner-host-advisory-sources.json` is now the repo-owned contract for later source-backed promotion decisions across `runner-images`, `android-java`, `android-gradle`, `android-emulator-runtime`, and `ios-xcode-simulator`; all five stay `manual-review-required` on current `main`
- those expansions intentionally stay scoped to dependency-graph-backed alerts, the explicit checked-in security-tooling inventory, and drift-triggered runner-host review rather than broad runner-image or generic downloaded-tool CVE scraping; later source-specific promotion work is split across #143, #154, #155, #156, and #144 while continuing to reuse the existing runner-host managed-issue lane
