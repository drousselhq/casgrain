...
- `.github/runner-host-advisory-sources.json` is now the repo-owned contract for runner-host source-backed promotion decisions: `runner-images` and `android-emulator-runtime` are promoted on current `main`, `android-java` and `android-gradle` remain `manual-review-required`, and the current combined `ios-xcode-simulator` placeholder stays manual-only while later iOS work remains under `#164` / `#165`
- those expansions intentionally stay scoped to dependency-graph-backed alerts, the explicit checked-in security-tooling inventory, and the bounded runner-host release-metadata/drift lane rather than broad runner-image or generic downloaded-tool CVE scraping; later source-specific promotion work now remains in #154, #155, #164, and #165 while the shipped `runner-images` and `android-emulator-runtime` slices continue to reuse the existing runner-host managed-issue lane
