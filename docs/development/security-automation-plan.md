...
- the workflow also evaluates the checked-in `.github/runner-host-watch.json` inventory against `host-environment.json` artifacts emitted by the Android and iOS smoke workflows, opening `security: runner-host review needed` when the inventoried runner/toolchain facts drift, when required host evidence is missing/unreadable, or when the promoted `runner-images` release metadata disagrees with the observed runner-image facts
- `.github/runner-host-advisory-sources.json` is now the repo-owned contract for runner-host source-backed promotion decisions: `runner-images` is promoted on current `main`, while `android-java`, `android-gradle`, and `android-emulator-runtime` remain `manual-review-required` follow-ups and current `main` still renders one combined `ios-xcode-simulator` placeholder mapped to historical issue `#144`
- those expansions intentionally stay scoped to dependency-graph-backed alerts, the explicit checked-in security-tooling inventory, and the bounded runner-host release-metadata/drift lane rather than broad runner-image or generic downloaded-tool CVE scraping; the open source-specific promotion backlog now lives in `#154`, `#155`, `#156`, plus the later planned iOS split follow-ups `#164` and `#165`, while the shipped `runner-images` slice continues to reuse the existing runner-host managed-issue lane and the combined iOS manifest entry remains current-main compatibility rather than proof that the split already landed
