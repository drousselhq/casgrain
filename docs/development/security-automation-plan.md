...
- the weekly `cve-watch` workflow writes self-contained workflow summaries and fails the scheduled run when a Rust dependency, non-Cargo Dependabot, or workflow-installed security-tooling advisory needs action
- scheduled CVE-watch output must not create, reopen, update, label, or close GitHub issues automatically; remediation work should enter GitHub as a maintainer/agent-authored PR or explicit follow-up, not as a bot-minted queue item
...
- the workflow still renders the checked-in `.github/runner-host-watch.json` inventory against `host-environment.json` artifacts emitted by the Android and iOS smoke workflows, but runner-host drift/source-rule output is report-only in the scheduled CVE workflow and no longer opens `security: runner-host review needed`
- `.github/runner-host-advisory-sources.json` remains the repo-owned source-rule contract for runner-host report rendering; it is not a backlog generator
