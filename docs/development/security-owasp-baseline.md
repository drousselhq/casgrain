...
4. the workflow also evaluates `.github/runner-host-watch.json`, the checked-in inventory of watched runner-image / host-toolchain facts for the Android and iOS smoke workflows:
   ...
   - `.github/runner-host-advisory-sources.json` is the repo-owned source-rule contract for runner-host promotion decisions; `runner-images` and `android-emulator-runtime` are now the delivered source-backed groups on `main`, `android-java` and `android-gradle` remain `manual-review-required`, and the current combined `ios-xcode-simulator` placeholder stays manual-only while later iOS ownership lives in `#164` / `#165` rather than closed issue `#144`
...
2. compare findings only against Casgrain's actual remaining manual surface area:
   - source-backed host-toolchain advisory evaluation beyond the delivered `runner-images` / `android-emulator-runtime` slices, as split across `#154`, `#155`, `#164`, and `#165` in repo docs and follow-up issue specs; current `main` still keeps one combined `ios-xcode-simulator` manifest placeholder until that later split lands
...
Tracked gap:
- later source-backed promotion work for the runner-host watch now stays split across the delivered `#143` (`runner-images`) and `#156` (`android-emulator-runtime`) slices plus the remaining follow-ups `#154` (`android-java`), `#155` (`android-gradle`), `#164` (`ios-xcode`), and `#165` (`ios-simulator-runtime`); current `main` still keeps one combined `ios-xcode-simulator` manifest placeholder until that later split lands
