...
- Casgrain's target merge-gate contract is both `ios-smoke` and `android-smoke` because the product-true merge gate is the mobile simulator/emulator-backed execution contract rather than a single-platform proof.
- Both `ios-smoke` and `android-smoke` are already live-required on `main`, and both workflows always report a PR status while self-skipping unaffected diffs so the required contexts remain enforceable without paying full mobile cost on unrelated changes.
- both mobile checks may self-skip for PRs that do not touch their owned platform or shared runtime paths, but they must always report a status so branch protection remains enforceable; qualifying pushes to `main` also run the device path so fresh host evidence exists after runner-host changes merge.
- the Android workflow artifacts remain a real contract: the run must archive either the success bundle (`trace.json`, plan, screenshot, emulator metadata, UI dumps) or a runner-managed failure bundle with `failure.json`, `evidence-summary.json`, and a stable `failure_class`, plus any diagnostics that truthfully exist for that class. Boot-readiness failures may legitimately have no foreground/UI diagnostics; later foreground, `ui-dump-failure`, selector, and text failures should preserve the referenced diagnostics when available, and `ui-dump-failure` should always preserve `ui-last.xml` (empty when the dump itself yielded no bytes) plus the latest truthful foreground snapshot when available. Missing both bundles should be treated as an explicit `artifact-contract-breach`.
