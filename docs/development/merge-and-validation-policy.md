...
- `ios-smoke` is already live-required on `main`, and `android-smoke` is the remaining `#79` close-out step now that this branch gives it always-reporting PR behavior; add it to the live ruleset only after that workflow lands on `main`.
