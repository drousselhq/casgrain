...
- both `ios-smoke` and `android-smoke` are required on PRs because the current product-true merge gate is the mobile simulator/emulator-backed execution contract rather than a single-platform proof.
- both required mobile checks may self-skip for PRs that do not touch their owned platform or shared runtime paths, but they must always report a status so branch protection remains enforceable; qualifying pushes to `main` also run the device path so fresh host evidence exists after runner-host changes merge.
