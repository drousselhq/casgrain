...
- [ ] 1.1 Update the existing issue-sync fixtures/tests so the current `schedule-shortfall` case asserts the stale tracker transition is wrong on current `main`.
- [ ] 1.2 Add at least one regression that starts from an already-managed blocker issue and proves the old tracker-centric plan would leave stale issue state behind.
- [ ] 1.3 Verify the new/updated tests fail on the pre-change implementation before changing production behavior.
...
- [ ] 2.1 Remove the required live-tracker assumption from `android_smoke_issue_sync.py` and its dry-run output.
- [ ] 2.2 Keep one managed-blocker path for concrete non-threshold blockers, including create/reopen/update behavior per blocker class.
- [ ] 2.3 Close a previously managed blocker issue when a later report no longer justifies keeping it open.
...
- [ ] 3.1 Update `.github/workflows/android-emulator-smoke.yml` so the sync step no longer passes `--tracker-issue 132`.
- [ ] 3.2 Keep the same event boundary for issue mutation and preserve the current human-readable report output.
- [ ] 3.3 Confirm the workflow text no longer depends on any dedicated live tracker issue for Android reliability state.
...
- [ ] 5.2 Re-run the local issue-sync unit tests.
- [ ] 5.3 Rebuild the live Android reliability summary and confirm the dry-run stays advisory-only for the current `schedule_main_runs_below_threshold` state.
