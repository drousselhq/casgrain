...
- the report must not imply broader Android source-backed advisory evaluation than current `main` ships; today `runner-images`, `android-java`, `android-gradle`, and `android-emulator-runtime` are active while the iOS split remains later follow-up work
