...
- [ ] 1.3 Prove the new assertions keep Java source-backed findings explicit in group/platform details and reuse the existing top-level `advisory_count` without inventing `source_advisory_count`.
...
- [ ] 2.3 Keep `runner-images` on the delivered `runner-image-release-metadata` rule, keep `android-gradle` on its delivered source-backed rule from `#155`, keep `android-emulator-runtime` as the remaining `manual-review-required` follow-up group, and leave the current combined `ios-xcode-simulator` placeholder unchanged in this slice.
...
- Validation:
  ...
  assert groups['android-emulator-runtime']['rule_kind'] == 'manual-review-required', groups['android-emulator-runtime']
...
- [ ] 3.4 Preserve the existing drift / missing-evidence behavior and keep top-level `advisory_count` aligned with current `main` by counting review-needed promoted source-backed findings there too, while keeping Java findings explicit in group/platform details without inventing `source_advisory_count`.
...
- [ ] 4.1 Update `docs/development/cve-watch-operations.md`, `docs/development/security-automation-plan.md`, and `docs/development/security-owasp-baseline.md` so they state that `runner-images` remains the delivered source-backed group, `android-gradle` is already source-backed on current `main`, `android-java` is newly source-backed in this slice, `android-emulator-runtime` remains a manual-review follow-up, and the current combined `ios-xcode-simulator` placeholder still remains `manual-review-required` under `#144` on current `main`.
