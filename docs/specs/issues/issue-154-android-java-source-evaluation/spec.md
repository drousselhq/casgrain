...
- Related follow-up issues that must remain separate from this slice:
  ...
  - `#164` / `#165` (`historical follow-up issue numbers for the current combined ios-xcode-simulator placeholder on main`)
  - `#164` (`historical ios-xcode source-backed follow-up issue number once the iOS split lands`)
  - `#165` (`historical ios-simulator-runtime source-backed follow-up issue number once the iOS split lands`)
...
- A fresh live invocation (`2026-04-23` UTC) against current `main` reports:
  ...
  - report title `security: runner-host review needed`
  - source-rule groups `runner-images`, `android-java`, `android-gradle`, `android-emulator-runtime`, `ios-xcode-simulator`
  - `runner-images` now evaluates as `runner-image-release-metadata`, `android-java` now evaluates as `java-release-support`, `android-gradle` now evaluates as `gradle-release-catalog`, `android-emulator-runtime` now evaluates as `android-system-image-catalog`, and the current combined `ios-xcode-simulator` placeholder stays manual-only while still carrying historical `#164` / `#165` references
- `docs/specs/issues/issue-144-ios-runner-host-source-split/{spec,tasks}.md` already describe the earlier iOS split into `#164` and `#165`; on current `main` those references are historical only, while the live report still keeps the combined `ios-xcode-simulator` placeholder instead of reviving closed issue `#144` as a current owner.
...
- preserve `android-gradle` as the delivered `gradle-release-catalog` group mapped to `#155`, treat `android-emulator-runtime` as an already-delivered source-backed slice under `#156`, and leave the current combined `ios-xcode-simulator` placeholder manual-only while it still carries historical `#164` / `#165` references
...
- the current combined `ios-xcode-simulator` placeholder group stays `manual-review-required` on current `main`; if mentioned, `#164` / `#165` are historical follow-up issue numbers rather than live owners, and this slice does not widen that placeholder
...
- `docs/specs/issues/issue-144-ios-runner-host-source-split/spec.md` must stop saying that the shipped runner-host automation still evaluates only drift / missing evidence; after `#154` lands it must describe `runner-images`, `android-gradle`, `android-emulator-runtime`, and `android-java` as the already-delivered source-backed exceptions while keeping the current combined iOS placeholder truthful and treating `#164` / `#165` only as historical follow-up issue numbers if mentioned
- `docs/specs/issues/issue-144-ios-runner-host-source-split/tasks.md` must stop telling implementers to expect split current-main iOS keys; until a later iOS split ships, that historical task artifact must preserve the live combined `ios-xcode-simulator` placeholder while treating `#164` / `#165` only as historical follow-up issue numbers
...
- **no** iOS source-rule split rework or source-backed evaluation; the current combined `ios-xcode-simulator` placeholder stays unchanged in this slice while any `#164` / `#165` references remain historical only
...
- the current combined `ios-xcode-simulator` placeholder remains manual-only and still carries historical `#164` / `#165` references
