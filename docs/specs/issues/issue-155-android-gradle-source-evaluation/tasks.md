...
- [ ] 4.5 Run a targeted search for stale wording that still claims `runner-images` is manual-only, that every runner-host group is manual-only, or that `#155` remains future work on current `main`.
