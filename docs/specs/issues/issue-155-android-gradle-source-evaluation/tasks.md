...
- [x] 2.3 Keep `runner-images` on its existing `runner-image-release-metadata` contract, and keep `android-java`, `android-emulator-runtime`, and `ios-xcode-simulator` as `manual-review-required` follow-up groups.
...
- [x] 5.5 In the PR summary/comment, say the implementation PR `Closes #155`, explicitly note that `docs-needed` still applies because canonical security docs changed, and state that `runner-images` stayed source-backed while the remaining follow-up groups stayed manual-only.
