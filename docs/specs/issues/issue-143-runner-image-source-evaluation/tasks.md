...
- [x] 4.2 Update `docs/development/cve-watch-operations.md`, `docs/development/security-automation-plan.md`, `docs/development/security-owasp-baseline.md`, `docs/specs/issues/issue-129-runner-host-advisory-source-rules.md`, `docs/specs/issues/issue-124-runner-host-drift-watch.md`, and `docs/specs/issues/issue-142-android-runner-host-source-split.md` so they state that `#143` delivered the first source-backed group (`runner-images`) and that current `main` now also keeps `android-java` delivered from `#154` and `android-emulator-runtime` delivered from `#156` while `#155` and the split iOS follow-ups `#164` / `#165` remain open.
- [x] 4.3 In the three older issue-spec docs, remove the stale future-work wording that still presents `#143` as an open follow-up or keeps the whole runner-host lane drift-triggered, and reconcile them so `#143` is documented as the first delivered runner-images slice while later/current ownership stays with delivered `#154` / `#156` and open `#155` / `#164` / `#165` as appropriate to current `main`.
...
- [x] 5.4 In the PR handoff comment, state that `#143` promoted the first source-backed runner-host group (`runner-images`), list the exact validation commands, note that docs still need review because the spec/reporting contract changed, and call out that current `main` now tracks the remaining follow-up work under `#155`, `#164`, and `#165` while `#154` and `#156` are already delivered.
