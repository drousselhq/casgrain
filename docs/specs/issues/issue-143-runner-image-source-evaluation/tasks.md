...
- Hand back if: `runner-images` is no longer owned by `#143`, the runner-host issue title changed, or the current-main contract no longer shows `runner-images`, `android-java`, `android-gradle`, and `android-emulator-runtime` as the delivered source-backed slices with only later iOS ownership under `#164` / `#165`.
