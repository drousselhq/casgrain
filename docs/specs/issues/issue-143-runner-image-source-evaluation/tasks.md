...
- [x] 5.4 In the PR handoff comment, state that `#143` promoted the first source-backed runner-host group (`runner-images`), list the exact validation commands, note that docs still need review because the spec/reporting contract changed, and call out that current `main` now tracks later follow-up work under `#154`, `#155`, `#164`, and `#165` while `#156` is already delivered.
