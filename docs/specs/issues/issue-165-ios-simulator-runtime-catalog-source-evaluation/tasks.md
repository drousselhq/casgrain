...
Historical note: issues `#165` and `#172` are closed `not planned`; this task list survives only as the last simulator-runtime-only proposal. Current `main` still keeps the combined `ios-xcode-simulator` placeholder, and any `#164` / `#165` references on runner-host output/docs are historical only.
...
- [ ] 1.2 Only if the split exists on current `main`, confirm `ios-simulator-runtime` is mapped to `#165`, `ios-xcode` is mapped to `#164`, and `simulator.device_name` still stays outside this historical slice; any future source-backed device-availability work would need a freshly shaped bounded issue because `#172` is closed `not planned`.
...
- Validation: `python3 -m unittest tests/scripts/test_runner_host_review_report.py`
- Non-goals: No device-availability activation, no new report title, no runtime freshness ratchet.
...
- Hand back if: The bounded runtime evaluator would require widening the watched inventory, redefining the report sync path, or absorbing Xcode/device-availability source families instead of staying inside the existing runner-host watch.

## 5. Historical repo-owned docs and earlier issue-spec reconciliation target
- [ ] 5.1 In the superseded proposal, `docs/development/cve-watch-operations.md`, `docs/development/security-automation-plan.md`, and `docs/development/security-owasp-baseline.md` would have been updated to say that `ios-simulator-runtime` was source-backed while `ios-xcode` remained separate work, device-availability stayed out of scope for this slice, and the non-iOS groups (`runner-images`, `android-java`, `android-gradle`, `android-emulator-runtime`) kept their already-delivered source-backed states; historical shaping used `#172` for later device-availability work, but `#172` is now closed `not planned`.
- [ ] 5.2 In that same superseded post-`#165` target, `docs/specs/issues/issue-124-runner-host-drift-watch.md`, `docs/specs/issues/issue-129-runner-host-advisory-source-rules.md`, `docs/specs/issues/issue-142-android-runner-host-source-split.md`, `docs/specs/issues/issue-143-runner-image-source-evaluation/{spec,tasks}.md`, `docs/specs/issues/issue-144-ios-runner-host-source-split/{spec,tasks}.md`, and the other named adjacent issue-spec artifacts would no longer read as if current `main` still had no active iOS source-backed evaluation, still left `#144` as the live umbrella owner, or still preserved closed `#144` as the remaining iOS follow-up.
- [ ] 5.3 In that superseded target, `docs/specs/issues/issue-164-ios-xcode-source-evaluation/{spec,tasks}.md` would stop preserving the post-`#164` current-main contract where `ios-simulator-runtime` stayed manual-only future work and all source-backed review findings flowed only through the shared top-level `advisory_count`; instead, those older artifacts would treat `ios-simulator-runtime` as already source-backed on current `main` and preserve the separate simulator-runtime source finding/count surface instead of forbidding `source_advisory_count`.
- [ ] 5.4 In that same superseded target, `docs/specs/issues/issue-154-android-java-source-evaluation/{spec,tasks}.md` would stop preserving the pre-split `ios-xcode-simulator -> #144` live-owner story once the split prerequisite was on current `main`, and would keep the shared summary contract truthful by preserving a separate simulator-runtime source finding/count surface instead of collapsing those findings into a drift-only top-level count.
- [ ] 5.5 In that same superseded target, `docs/specs/issues/issue-155-android-gradle-source-evaluation/{spec,tasks}.md` would stop requiring all source-backed review findings to live only in the shared top-level `advisory_count` with no separate source-backed finding/count field.
- [ ] 5.6 In that same superseded target, `docs/specs/issues/issue-156-android-emulator-runtime-source-evaluation/{spec,tasks}.md` would stop requiring that shared top-level-count-only contract, and `issue-156/spec.md` would no longer say the later iOS work had open spec-entry PRs `#171` and `#173`.
- [ ] 5.7 The superseded proposal would also have made the docs explicit that `simulator.device_name` remained a drift-only supporting fact in this slice and that any future device-availability source automation would require a freshly shaped bounded issue rather than reusing closed `#172`.
- [ ] 5.8 The superseded validation would have run a targeted search for stale wording that still claimed `ios-simulator-runtime` was manual-only future work, that `#144` still owned the live iOS umbrella contract after the split prerequisite, that `issue-143/tasks.md` still said `#154`, `#155`, `#156`, and `#144` remained future work instead of the split `#164` / `#165` ownership, that `issue-164/{spec,tasks}.md` still preserved the post-`#164` top-level-`advisory_count`-only contract or kept `ios-simulator-runtime` manual-only, that adjacent `#154` / `#155` / `#156` specs/tasks still encoded conflicting shared summary/count expectations about `source_advisory_count`, or that `issue-156/spec.md` still preserved `#173` as an open spec-entry PR.
- Goal: Record the repo-contract changes this superseded proposal would have required, without leaving the artifact sounding like live backlog work.
- Validation:
  ```bash
  python3 - <<'PY'
  from pathlib import Path
...
- [ ] 6.5 In the historical PR summary/comment for this superseded proposal, record the closure target as `Closes #165`, explicitly note that `docs-needed` still applied because canonical security docs changed, and state that `#164` remained paired historical context while any future device-availability work would require a freshly shaped bounded issue instead of reusing closed `#172`.
