...
- [ ] 5.1 In the superseded proposal, `docs/development/cve-watch-operations.md`, `docs/development/security-automation-plan.md`, and `docs/development/security-owasp-baseline.md` would have been updated to say that `ios-simulator-runtime` was source-backed while `ios-xcode` and `#172` remained follow-up work, and the non-iOS groups (`runner-images`, `android-java`, `android-gradle`, `android-emulator-runtime`) kept their already-delivered source-backed states.
...
- [ ] 5.7 The superseded proposal would also have made the docs explicit that `simulator.device_name` remained a drift-only supporting fact in this slice and that any future device-availability source automation belonged to `#172`.
