...
- [ ] 3.3 Keep `ios-simulator-runtime` as the remaining `manual-review-required` iOS follow-up under `#165`, keep `runner-images`, `android-java`, and `android-emulator-runtime` on their already-delivered source-backed groups, and keep `android-gradle` as the remaining manual-review follow-up group.
