...
- [ ] 1.3 Verify the canonical docs named in `spec.md` still contain the specific stale current-main claims this slice must remove, including `docs/validation.md` treating `android-smoke` as the remaining `#79` promotion and describing the product-true proof as one fixture-specific iOS scenario, `docs/development/merge-and-validation-policy.md` treating `android-smoke` as the remaining `#79` close-out step, and `docs/development/test-pyramid-and-runtime-contracts.md` keeping Layer 4 focused on an iOS-first current slice while Android parity matures separately.
- [ ] 1.4 Verify both `docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/spec.md` and `docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/tasks.md` still preserve stale downstream framing for `#135` or rollout-pending Android gate wording so the repair scope stays grounded on a real current-main contradiction.
...
- [ ] 4.2 Update `docs/validation.md`, `docs/development/merge-and-validation-policy.md`, and `docs/development/test-pyramid-and-runtime-contracts.md` so they stop presenting `android-smoke` as future `#79` promotion work, stop describing the current proof as an iOS-first slice, and instead match the live rule that both `ios-smoke` and `android-smoke` are required mobile merge-gate checks on `main`.
...
- [ ] 4.6 Reconcile `docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/tasks.md` so it no longer preserves rollout-pending Android gate text, a still-pending live ruleset-flip goal, or post-merge close-out steps as current truth once this slice lands on `main`.
- [ ] 4.7 Keep archived history docs untouched unless a touched canonical doc explicitly points at them as current truth.
- Goal: Leave one truthful repo-wide description of the shipped bounded vertical slice and the live merge gate around it.
- Validation: `git grep -n -E 'iOS-first|parallel emulator-backed evidence lane|remaining required merge gate promotion for `#79`|first product-true execution proof is intentionally narrow: one fixture-specific iOS scenario|matching required check immediately after that workflow lands on `main`|remaining `#79` close-out step|Android parity matures separately|Bring the Android product-true vertical slice to parity with iOS|later parity work against the now-required Android gate|#135 stays downstream|live `android-smoke` ruleset flip is still pending|keep `#79` open until that verification is true on `main`' -- docs/specs/casgrain-product-spec.md docs/validation.md docs/development/merge-and-validation-policy.md docs/development/test-pyramid-and-runtime-contracts.md docs/development/security-owasp-baseline.md docs/prd/product-requirements.md docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/spec.md docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/tasks.md`
