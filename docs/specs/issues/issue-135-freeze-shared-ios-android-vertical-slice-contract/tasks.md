...
- [x] 4.5 Reconcile `docs/specs/issues/issue-79-promote-android-smoke-to-a-required-merge-gate-on-main/spec.md` so it stops presenting `#135` as a future parity/unblock follow-up or ongoing `#79` completion seam, and instead records `#135` only as the separate docs/test contract-freeze follow-up that closes through its own PR.
