...
- [x] 4.1 Update `docs/development/cve-watch-operations.md`, `docs/development/security-automation-plan.md`, and `docs/development/security-owasp-baseline.md` so they state that `runner-images` remains the delivered source-backed group, `android-emulator-runtime` is newly source-backed in this slice, `android-java` remains the manual-review follow-up, `android-gradle` is already source-backed on current `main`, and current docs do not preserve closed issue `#144` as the live owner of the later iOS work already split across `#164` / `#165`.
