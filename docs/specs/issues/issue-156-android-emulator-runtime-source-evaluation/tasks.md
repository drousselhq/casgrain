...
- [x] 5.5 In the PR summary/comment, say the implementation PR `Closes #156`, explicitly note that `docs-needed` still applies because canonical docs/specs changed, and state that `runner-images` remains delivered, `android-java` / `android-gradle` remain open follow-up scopes, and later iOS work stays with `#164` / `#165`.
