...
- [ ] 1.1 Update `tests/scripts/test_mobile_smoke_workflow_triggers.py` so current `main` fails if `android-emulator-smoke.yml` still relies on `pull_request.paths` instead of always reporting an `android-smoke` context on PRs.
- [ ] 1.2 Add or extend fixture/assertion coverage for the Android/shared-runtime path list the workflow should treat as `should_run=true`.
- [ ] 1.3 Verify the new assertions fail on the pre-change workflow before editing production YAML.
...
- [ ] 2.1 Update `.github/workflows/android-emulator-smoke.yml` so the workflow runs on every PR and keeps `jobs.smoke.name` exactly `android-smoke`.
- [ ] 2.2 Add in-job change detection that uses the existing Android/shared-runtime surface list to decide whether the expensive emulator path is required.
- [ ] 2.3 When `should_run=false`, report success with a clear skip message and do not build the APK, boot the emulator, or upload Android smoke artifacts.
- [ ] 2.4 When `should_run=true`, preserve the current emulator-backed smoke run, artifact validation, reliability report, and issue-sync behavior.
...
- [ ] 4.1 Record the current `main-protection-ruleset` required status-check list before editing it.
...
- [ ] 5.1 Run `git diff --check` and the workflow-trigger regression tests.
- [ ] 5.2 Re-read the final Android workflow YAML and confirm the required-check-safe trigger plus stable `android-smoke` job name still hold.
