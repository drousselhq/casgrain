...
- `android-emulator-smoke` is advisory for merge gating today: it auto-runs for Android/shared-runtime changes, qualifying pushes to `main`, and the nightly drift-catching schedule, but it is not yet a required branch-protection gate.
- Even while Android smoke is advisory for merge gating, the workflow still has a required artifact/evidence contract when it runs. The job must archive either the success bundle (`trace.json`, plan, screenshot, emulator metadata, UI dumps) or a runner-managed failure bundle with `failure.json`, `evidence-summary.json`, and a stable `failure_class`, plus any diagnostics that truthfully exist for that class. Boot-readiness failures may legitimately have no foreground/UI diagnostics; later foreground, `ui-dump-failure`, selector, and text failures should preserve the referenced diagnostics when available, and `ui-dump-failure` should always preserve `ui-last.xml` (empty when the dump itself yielded no bytes) plus the latest truthful foreground snapshot when available. Missing both bundles should be treated as an explicit `artifact-contract-breach`.
- Android reliability qualification is tracked separately from branch protection through the repo-owned report/sync path that targets tracker issue `#132`. The current `schedule_main_runs_below_threshold` result is therefore a live tracker condition, not by itself a blocker issue or a reason to promote Android to a required merge gate; any later gate promotion still belongs to `#79`.
