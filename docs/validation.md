...
- `ios-simulator-smoke` now runs on every PR, but self-skips for obvious docs-only / governance-only diffs; this keeps the required `ios-smoke` check present without paying the full simulator cost on clearly non-runtime changes
- `android-emulator-smoke` runs automatically on PRs only when Android/shared-runtime paths change, and both mobile smoke workflows also run on qualifying pushes to `main` plus a nightly schedule so `cve-watch` can consume fresh `main` host evidence after runner-host changes land
...
- The repo-owned reliability report/sync path keeps the current qualification state current in issue `#132`; a `schedule_main_runs_below_threshold` shortfall stays tracker-only and does not itself promote Android to a required gate.
