...
const DEFAULT_SMOKE_SCRIPT: &str = "tests/scripts/android_smoke_run_plan.py";
...
fn resolve_repo_root() -> Result<PathBuf, RuntimeFailure> {
    ...
    Err(RuntimeFailure {
        code: FailureCode::EngineError,
        message: String::from(
            "failed to locate repository root; set CASGRAIN_REPO_ROOT to a checkout containing tests/scripts/android_smoke_run_plan.py",
        ),
    })
}
    ...
    fn supported_smoke_plan() -> ExecutablePlan {
        ExecutablePlan {
            ...
            source: PlanSource {
                ...
                source_name: "tests/fixtures/android-smoke/features/tap_counter.feature".into(),
...
"##,
    ...
    fn install_foreign_anr_overlay_adb(repo_root: &Path) -> PathBuf {
        fs::create_dir_all(repo_root.join("scripts")).expect("repo root should be created");
